...
- **段落閱讀書籤**：每篇教材可保留一個書籤。每個英文段落左側留白都常駐書籤按鈕，點未選取項目即可建立或移動唯一書籤，點已選取項目則移除。若複製或螢光筆正啟用，點擊任一段落書籤或翻譯按鈕都會先結束該工具，再執行所選操作。工具列的返回書籤按鈕只負責導航：有書籤時啟用並返回該段，沒有書籤時保持可見但停用。返回首頁後再次進入或重新載入教材頁，所有段落書籤按鈕仍會顯示。書籤寫入期間會防止重複操作，但不會讓整排按鈕同步變暗閃爍。英文與中文共用固定左右縮排，書籤只位於左側留白，不會遮住或推動正文。移除或往前移動書籤不會撤銷已認識單字。
...
- 閱讀頁會辨識英文正文與相鄰中文翻譯；單一懸浮工具列提供全篇翻譯開關、一次性段落複製、全篇螢光筆與返回閱讀書籤。各英文段落左側留白常駐書籤按鈕，可直接建立、切換或移除；建立或往後移動書籤會同步累積讀到該段的英文正文單字，頁底也可一次補完剩餘進度。
