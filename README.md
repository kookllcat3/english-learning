...
- **段落閱讀控制**：每段原文尾端提供「整段標記為認識」與「目前閱讀位置」按鈕；閱讀位置可取消、跨段互斥，重新開啟素材後仍會保留。只要素材設有閱讀位置，「素材內容」標題旁便會顯示「回到閱讀位置」；匯入備份時也會確認標記確實指向現有段落，避免損壞標記留下無效按鈕。
...
- **筆記操作更專注**：暫時隱藏使用率較低的格式工具列，保留筆記輸入、自動儲存與備份，減少閱讀時的操作干擾。
...
- 自動整理素材中的英文詞彙，分別記錄每份素材的認識狀態、完成度與全域熟悉度。
...
- 每段原文尾端可一次將該段單字標記為認識，或設定目前閱讀位置；全篇最多標記一段，也可清除全部標記。已標記段落會有高對比書籤與段落標線；設定閱讀位置後，可隨時從「素材內容」標題旁快速回到該段，清除標記後按鈕才會隱藏。
- 中文及中英混合的解釋可逐段模糊，桌面停留 0.6 秒或手機觸碰後可切換該段顯示狀態。
- 素材製作指南提供純文字與圖文 DOCX 兩種 AI 教材提示詞；閱讀頁也能產生包含素材全文的學習提示詞。
- `.elpkg` 完整備份包含素材、圖片、學習進度、閱讀位置、單字筆記及設定；匯入會先驗證跨資料關聯、預覽並合併資料，不會直接清空現有內容。閱讀位置若未對應素材中的實際段落，整份備份會在寫入前被拒絕。
...
- 備份：介面接受 100 MB 以內的 `.elpkg` 或舊版 JSON；封裝內容還會檢查項目數、單檔及累積解壓容量，超限或驗證失敗時不會寫入資料
