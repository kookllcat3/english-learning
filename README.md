...
- **操作更穩定**：修正卡片遮擋文字、點擊穿透、手機版捲動，以及長標題與彈窗版面等問題，閱讀與操作更順暢。
...
- 桌面 hover 持續 1 秒後顯示單字卡；字卡開啟後移到其他單字會立即切換，關閉後重新套用 1 秒延遲；觸控、雙擊或選取單字可直接開啟，框選文字開啟時會自動釘選，並提供瀏覽器發音、認識狀態與 Markdown 筆記
- 素材製作指南提供可一鍵複製的 AI 教材生成提示詞，並可切換純文字與圖文版本
- 將素材全文整理成可編輯、可交給任意 AI 服務的學習提示詞
- 匯出及匯入包含圖文素材、單字筆記與設定的版本化 `.elpkg` 備份封裝
- 響應式支援桌面與手機版面
