...
這次更新完成前端架構升級，並改善桌面與手機上的操作流暢度：

- 全站介面改由 Vue 3、Vue Router、Pinia 與嚴格 TypeScript 管理，既有 IndexedDB 資料與備份格式維持相容。
- 首頁、素材閱讀、資料管理與各類對話框統一為 Vue 元件，舊素材網址仍會自動轉址。
- 素材卡片維持一致高度，搜尋、排序及窄螢幕分頁切換不再造成明顯版面跳動。
- 對話框加入輕量過場、正確焦點管理及 reduced-motion 支援，長內容只在必要的內部區域捲動。
- 素材製作指南重新排版，AI 教材生成提示詞可切換「純文字／圖文」版本並一鍵複製。
- 首頁採摘要與分頁載入，長詞彙列表分批渲染，圖文素材只為接近可視區域的圖片建立 Blob URL。
- 新增桌面與 390 px 窄螢幕的效能、無障礙、資料持久化及正式建置自動驗證。
