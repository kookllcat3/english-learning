...
- **單一懸浮閱讀工具列**：教材頁不再於每個段落重複顯示工具；翻譯、複製、螢光筆與閱讀書籤集中在正文第一列，往下閱讀時會固定在頁首下方。翻譯按鈕一次隱藏或顯示整篇中文，複製按鈕則進入一次性選段模式，複製結果會由畫面下方的短暫 Snackbar 回饋，不擠動閱讀工具列。
- **全篇螢光筆**：啟用一次即可在任意英文段落點擊或拖曳標記；點中文、正文外區域、再次按工具或按 Esc 會結束模式，不會誤開單字卡。
- **段落閱讀書籤**：每篇教材可保留一個書籤。進入設定模式後，每個英文段落左側留白都會顯示書籤按鈕；點選即可建立、移動或移除，並可在不同段落間連續切換。若複製或螢光筆正啟用，點擊工具列書籤、任一段落書籤或翻譯按鈕，都會先自動結束該工具，再執行所選操作，避免同一次點擊具有兩種意圖。切換複製或螢光筆、返回首頁後再次進入教材，或重新載入教材頁時，這些書籤按鈕都會持續顯示，不必重新進入設定模式。書籤寫入期間仍會防止重複操作，但不會讓整排按鈕同步變暗閃爍。已有書籤時，工具列按鈕會直接返回該段；沒有書籤時再次按下工具列按鈕會退出設定模式。英文與中文共用固定左右縮排，書籤只位於左側留白，不會遮住或推動正文。移除或往前移動書籤不會撤銷已認識單字。

### 2026.08.09（v2.0.0）

- **英文正文螢光標記**：提供單一半透明淡黃色螢光筆。啟用後，從未標記單字落筆會上色，從已標記單字落筆會擦除；PC 可點擊或按住滑鼠拖曳，手機以 touch 點字，鍵盤則支援 Enter。可跳過不相鄰單字建立同一群組，重新整理後仍會保留標記。
...
- **完成度排序更穩定**：首頁選擇「完成度」時會先依完成比例由低到高排列；完成度相同的教材再依加入時間由新到舊排列，不會因後續編輯時間改變順序。
...
- 閱讀頁會辨識英文正文與相鄰中文翻譯；單一懸浮工具列提供全篇翻譯開關、一次性段落複製、全篇螢光筆與單一閱讀書籤。進入設定模式後可直接點選各英文段落左側留白的書籤按鈕來建立、切換或移除；建立或往後移動書籤會同步累積讀到該段的英文正文單字，頁底也可一次補完剩餘進度。
...
- 移除教材與重算共用詞彙狀態採同一個 IndexedDB 交易；任一步驟失敗時，教材與詞彙狀態都會維持操作前內容。
