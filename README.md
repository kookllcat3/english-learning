...
- **備份匯入失敗回饋**：匯入會顯示讀取、驗證與寫入進度；圖片資產會以 ArrayBuffer 寫入 IndexedDB，讀取時仍相容舊版 Blob。無法支援的圖片教材會自動略過並提示數量；其他 IndexedDB 交易或儲存空間失敗則會以可讀錯誤告知，並保留匯入前資料不變。
...
- `.elpkg` 完整備份包含素材、圖片、學習進度、閱讀位置、單字筆記及設定；匯入會先驗證跨資料關聯、預覽並合併資料，不會直接清空現有內容。無法支援的圖片或超過單份素材限制的教材會在確認前列為略過，不影響其他教材匯入；真正損壞的備份、閱讀位置若指向不存在的段落，仍會在寫入前拒絕整份備份。舊標記若指向現在辨識為標題或補充內容的既有段落，會安全清除。
