...
- **單一懸浮閱讀工具列**：教材頁不再於每個段落重複顯示工具；翻譯、複製、螢光筆與閱讀錨點集中在正文第一列，往下閱讀時會固定在頁首下方。翻譯按鈕一次隱藏或顯示整篇中文，複製按鈕則進入一次性選段模式。
- **全篇螢光筆**：啟用一次即可在任意英文段落點擊或拖曳標記；點中文、正文外區域、再次按工具或按 Esc 會結束模式，不會誤開單字卡。
- **段落閱讀錨點**：每篇教材可保留一個錨點。沒有錨點時可選擇任意英文段落建立；已有錨點時工具按鈕可直接返回，段落開頭的書籤選單則可移動或移除。移動失敗或取消會保留舊位置，移除與往前移動不會撤銷已認識單字。
...
- 閱讀頁會辨識英文正文與相鄰中文翻譯；單一懸浮工具列提供全篇翻譯開關、一次性段落複製、全篇螢光筆與單一閱讀錨點。建立或往後移動錨點會同步累積讀到該段的英文正文單字，段落開頭的書籤可移動或移除錨點，頁底也可一次補完剩餘進度。
