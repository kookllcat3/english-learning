...
- **備份匯入失敗回饋**：匯入會顯示讀取、驗證與寫入進度；若圖片 Blob、瀏覽器儲存空間或 IndexedDB 交易失敗，會以可讀的錯誤訊息告知，並保留匯入前資料不變。
...
- 備份：介面接受 100 MB 以內的 `.elpkg` 或舊版 JSON；封裝內容還會檢查項目數、單檔及累積解壓容量，超限、驗證或 IndexedDB 寫入失敗時會顯示錯誤，且不會留下部分資料
