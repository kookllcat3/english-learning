...
本機網址固定使用 `127.0.0.1:4173`，以維持同一份瀏覽器資料。重複啟動時，啟動器會直接開啟已在執行的英文學習庫；若該 port 被其他程式占用，則會顯示占用程序並停止，請關閉該程序後重新執行 `start.cmd`。
...
匯入解析、備份還原、單字筆記及文字規則，不直接建立或更新頁面 DOM。
...
- 每次匯出的 `.elpkg` 都包含經 manifest 宣告及 SHA-256 驗證的 `AI_README.md`，說明教材、詞彙涵蓋、跨教材熟悉度、單字筆記與標記應如何關聯解讀；`knownWords`、`knownCount` 與 `learned` 是教材進度涵蓋狀態，不代表背熟或測驗通過。
- 網站不會自動把備份傳送給 AI 或其他外部服務；使用者自行上傳 `.elpkg` 前，應確認備份內的教材、筆記、搜尋紀錄與設定是否適合交由該服務處理。
...
- 匯入備份時會先顯示備份與目前資料數量；確認後以備份完整覆蓋本機資料庫，任一寫入失敗時全部回滾。
- 舊備份若含已無法對應目前段落的教材標記，會先依教材內容修復；完全失效的單筆標記會略過，不影響其他資料還原。格式損壞、重複識別碼或封裝校驗失敗仍會拒絕整份備份。
...
- TXT：UTF-8 純文字，內容上限 2 MB；支援 LF、CRLF 與 CR 換行；連續完整英文句即使只以單一換行分隔，也會各自成為閱讀段落；英文與中文翻譯仍可使用相鄰行配對；無效 UTF-8 會拒絕且不寫入
- 教材匯出：目前僅提供不含圖片教材的 UTF-8 TXT
- 備份：介面接受 100 MB 以內的 `.elpkg` 或舊版 JSON；新匯出封裝包含 `AI_README.md`，舊封裝不需要此檔案仍可匯入；封裝內容還會檢查項目數、單檔及累積解壓容量，超限、驗證或 IndexedDB 寫入失敗時會顯示錯誤，無法支援的個別教材則自動略過並顯示數量
...
彈窗回歸測試同時驗證背景點擊關閉，以及從文字欄位拖曳到背景時不會誤關閉。
...
修改功能後，至少應在瀏覽器驗證 TXT 教材新增與更新、既有圖文教材閱讀與匯出、重新命名、刪除、正文純文字呈現、單字卡熟練度、教材庫搜尋與分頁及備份還原。
