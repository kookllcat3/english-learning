...
- **段落閱讀書籤**：每篇教材可保留一個書籤。進入設定模式後，每個英文段落左側留白都會顯示書籤按鈕；點選即可建立、移動或移除，並可在不同段落間連續切換。若複製或螢光筆正啟用，點擊任一段落書籤按鈕會先自動結束該工具，讓書籤操作保持明確。切換複製或螢光筆、返回首頁後再次進入教材，或重新載入教材頁時，這些書籤按鈕都會持續顯示，不必重新進入設定模式。書籤寫入期間仍會防止重複操作，但不會讓整排按鈕同步變暗閃爍。已有書籤時，工具列按鈕會直接返回該段；沒有書籤時再次按下工具列按鈕會退出設定模式。英文與中文共用固定左右縮排，書籤只位於左側留白，不會遮住或推動正文。移除或往前移動書籤不會撤銷已認識單字。
