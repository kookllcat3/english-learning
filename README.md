...
- **單字卡操作更精簡**：移除卡片內的認識狀態切換；發音與釘選按鈕並列，認識狀態仍可在教材詞彙頁管理。
- **快速查字不再閃爍**：單字卡會先定位並顯示，再載入共用筆記內容；快速移到不同單字時不必等待每次讀取完成。

### 2026.08.05（v1.7.1）

- **避免誤把整段單字設為認識**：閱讀段落不再提供整段批次標記；仍可透過單字卡逐詞設定，或在教材詞彙頁一次勾選需要的單字。
- **中英文段落控制外觀一致**：中文解釋的編輯與顯示切換按鈕，改為與英文段落的複製及閱讀位置按鈕相同版型；功能、鍵盤操作與儲存方式不變。
...
- 桌面停留單字 0.6 秒後開啟單字卡；觸控、雙擊或框選文字也能直接開啟。卡片會先定位並顯示，接著載入共用筆記內容，因此快速切換單字不會等待資料讀取。卡片提供發音、釘選及 Markdown 筆記；認識狀態可在教材詞彙頁管理。同一單字的筆記在所有教材與查詢入口共用。筆記編輯器取得焦點、或在卡片標題開始拖曳框選時，卡片會自動釘選，IME 組字期間不會被短暫失焦中斷。桌面開卡時保留頁面自然捲動，觸控裝置則鎖住後方頁面，卡片內筆記區仍可獨立捲動；釘選後點擊或觸碰面板外空白處會先取消釘選，再依非釘選規則收合，點擊另一個單字則立即換卡。
- 每段英文正文尾端可複製完整英文段落或設定目前閱讀位置；複製內容不包含中文解釋，並會就地顯示成功或失敗狀態。全篇最多標記一段，也可清除全部標記。已標記範圍會以高對比書籤與段落標線涵蓋英文正文，不包含中文解釋；設定閱讀位置後，可從「教材內容」標題旁快速回到該段，向下捲過標題後按鈕會懸浮在頁首下方，清除標記後才會隱藏。閱讀段落不提供整段批次認識功能，單字認識狀態仍可從單字卡或教材詞彙頁管理。
- 直接緊接英文正文的純中文行或文字區塊會視為該段翻譯；短句不需要中文前綴、句尾標點或最小字數。中文解釋可逐段編輯與模糊，編輯時可用「儲存／取消」或 Ctrl/Cmd+Enter 儲存、Escape 取消；修改只更新該段 translation 行，不會把中文算入英文單字。桌面停留 0.6 秒或手機觸碰後可切換顯示狀態；標題、雙語索引、`WORD POWER`、學習方法、單字補充及文法補充保持一般可選取文字。
- 教材製作指南提供純文字與圖文 DOCX 兩種可編輯的 AI 教材提示詞；兩者分別自動儲存在 IndexedDB 並包含於完整備份。閱讀頁也能產生包含教材全文的學習提示詞。
- `.elpkg` 完整備份包含教材、圖片、學習進度、閱讀位置、同單字共用筆記及設定；匯入會先驗證、預覽並合併資料，不會直接清空現有內容。單份教材的圖片、欄位或閱讀位置關聯錯誤會在確認前列為略過，不影響其他教材匯入；封裝結構、共用詞彙／設定資料或 IndexedDB 寫入失敗，才會拒絕整份備份。舊版全域筆記也可還原；既有位置型筆記保留為相容資料，不會被轉換成共用筆記。
