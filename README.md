...
- **教材資料影響提示**：更新或移除教材前，會完整說明內容、圖片、閱讀位置、標記、共用筆記與已認識單字將如何處理。
- **教材段落辨識修正**：支援 LF、Windows CRLF 與舊式 CR 換行；同一文字區塊若出現逐行英中翻譯，會自動拆成獨立的英文學習段落，同時保留一般多行英文散文的原段落。
...
- 匯入 UTF-8 TXT，或直接貼上文字建立教材。
- 教材庫提供搜尋、分段式排序與學習進度摘要；純文字教材可匯出 TXT，也可用 TXT 更新同一份教材，不會建立重複項目。含圖片教材的 DOCX 匯出與更新入口目前暫停。排序選取只改變選項內部底色，正文與單字卡標題的熟悉度使用固定深靛藍描邊與淡金流光，並與半透明閱讀標記背景清楚分層。
- 閱讀頁會辨識英文正文與相鄰中文翻譯；逐行英中內容會依英文行與其後翻譯建立獨立學習段落，一般多行英文散文則維持原段落。單一懸浮工具列提供全篇翻譯開關、一次性段落複製、全篇螢光筆與返回閱讀書籤。各英文段落左側留白常駐書籤按鈕，可直接建立、切換或移除；建立或往後移動書籤會同步累積讀到該段的英文正文單字，頁底也可一次補完剩餘進度。
